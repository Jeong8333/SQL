...
INNER JOIN departments b
USING(department_id) 
WHERE department_id = 30;  -- using 절에 사용한 컬럼은 테이블명에 들어가면 안됨
...
--CROSS JOIN
SELECT *
FROM tb_a, tb_b;    -- 3 * 3 행 출력됨.
SELECT *
FROM tb_a
CROSS JOIN tb_b;


-- 학생의 수강 내역을 출력하시오 LEFT OUTER JOIN
-- 과목이름
SELECT a.이름
     , b.강의실
     , b. 과목번호
...
-- 사용 목적 : 데이터 보안 측면, 복잡하고 자주 사용하는 쿼리를 간단하게.
-- VIEW 생성 권한이 없다면
...
-- java게정에서 member 계정에게 뷰 조회권한 부여
...
/*
    스키마
    데이터베이스 구조와 제약 조건에 관한 전반적인 며세를 기술한 메타데이터 집합
...
/*
    ...
    PUBLIC 모든 사용자 접근 가능, PRIVATE 특정 사용자만
...
INCREMENT BY 1  -- 증강숫자
START WITH 1    -- 시작숫자
MINVALUE   1    -- 최소(시작숫자와 같아야 함)
MAXVALUE   10   -- 최대(시작보다 커야함)
NOCYCLE         -- 최대 or 최소 도달시 멈추도록
NOCACHE;        -- 메모리에 미리 값을 할당하지 않도록 nocache로 안하면 건너뜀(미리 메모리에 할당해서)
